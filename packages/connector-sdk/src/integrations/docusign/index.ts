  ...
  actions: [
    {
      ...
      inputSchema: {
        type: 'object',
        required: ['accountId', 'emailSubject', 'signerEmail', 'signerName', 'documentBase64', 'documentName'],
        properties: {
          accountId:      { type: 'string', title: 'DocuSign Account ID',                description: 'Your DocuSign account ID from the admin portal' },
          emailSubject:   { type: 'string', title: 'Email Subject Line',                 description: 'Subject of the signature request email' },
          signerEmail:    { type: 'string', title: 'Signer Email',                       description: 'Email address of the person signing the document' },
          signerName:     { type: 'string', title: 'Signer Full Name',                   description: 'Full name of the signer' },
          documentBase64: { type: 'string', title: 'Document File Base64 Content',       description: 'Base64-encoded PDF content to send for signature' },
          documentName:   { type: 'string', title: 'Document File Name (e.g. Contract.pdf)', description: 'File name for the document' },
        },
      },
      outputSchema: {
        type: 'object',
        properties: {
          envelopeId: { type: 'string', title: 'Envelope ID' },
          status:     { type: 'string', title: 'Envelope Status' },
        },
      },
      inputs: [
