...
export default function RegisterPage() {
  ...
  const handleSelectGoogleAccount = async (email: string, name: string) => {
    ...
    try {
      ...
      toast.success(`Registered with Google Account`, {
        description: `Welcome, ${registeredUser.name} (${registeredUser.email}). Google Workspace connectors ready.`,
      });
    ...
    <div className="min-h-screen bg-bgPrimary flex flex-col items-center justify-center p-4">
      ...
      <div className="w-full max-w-md bg-bgSecondary border border-borderColor/80 rounded-2xl shadow-2xl p-8 relative">
        <div className="text-center mb-6">
          ...
          <h1 className="text-xl font-bold text-white mb-1">Create Account with Google</h1>
          <p className="text-xs text-textSecondary">
            Instant 1-click workspace registration using your Google Account.
          </p>
        </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 z-50 animate-fadeIn">
          <div className="w-full max-w-sm bg-white text-gray-900 rounded-2xl shadow-2xl p-6 relative border border-gray-200">
            <button
              onClick={() => { setIsModalOpen(false); setCustomEmail(''); setCustomName(''); }}
            ...
            <div className="text-center mb-5">
              ...
              <h2 className="text-base font-bold text-gray-900">Sign up with Google</h2>
              <p className="text-xs text-gray-500">Enter your Google Account email to create account</p>
            </div>
            ...
            >
              <div>
                <label className="text-[11px] font-semibold text-gray-700 block mb-1">Google Email Address:</label>
                <input
                  type="email"
                  placeholder="yourname@gmail.com"
                  value={customEmail}
                  onChange={(e) => setCustomEmail(e.target.value)}
                  className="w-full bg-gray-50 border border-gray-300 rounded-lg px-3 py-2 text-xs outline-none focus:border-blue-500 focus:bg-white transition-all text-gray-900"
                  required
                />
              </div>
              <div>
                <label className="text-[11px] font-semibold text-gray-700 block mb-1">Full Name (Optional):</label>
                <input
                  type="text"
              ...
              <button
                type="submit"
                disabled={isSubmitting}
                className="w-full py-2.5 bg-blue-600 text-white font-semibold text-xs rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50 mt-1 shadow-sm"
              >
                {isSubmitting ? 'Registering...' : 'Continue with Google Account'}
              </button>
            </form>
